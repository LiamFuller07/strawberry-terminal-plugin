...
import { ComputerUseAgent } from './computer-use.js';
...
export class BrowserController extends EventEmitter {
     ...
     */
    async executeTask(id, task, onProgress) {
        const instance = this.instances.get(id);
        if (!instance) {
            throw new Error(`Browser ${id} not found`);
        }
        ...
        try {
            const result = await instance.agent.executeTask(task, () => this.getScreenshot(id), (action) => this.executeAction(id, action), {
                maxSteps: 15,
                displaySize: {
