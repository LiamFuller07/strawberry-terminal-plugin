 ...
 */
async function runClawdbot(args, timeout = 60000) {
            ...
            try {
                switch (name) {
                    ...
                    case 'clawdbot_send_message': {
                        const { channel, target, message } = args;
                    ...
                    case 'clawdbot_agent': {
                        const { message, session_id, thinking, deliver, reply_channel, reply_to } = args;
                        const cmdArgs = ['agent', '--message', message, '--json'];
                        if (session_id)
                            cmdArgs.push('--session-id', session_id);
                        if (thinking)
                            cmdArgs.push('--thinking', thinking);
                        if (deliver)
                            cmdArgs.push('--deliver');
                        if (reply_channel)
                            cmdArgs.push('--reply-channel', reply_channel);
                        if (reply_to)
                            cmdArgs.push('--reply-to', reply_to);
                    ...
                    case 'clawdbot_memory_search': {
                        ...
                        if (limit)
                            cmdArgs.push('--limit', String(limit));
                        const result = await runClawdbot(cmdArgs);
                        return this.textResult(result.stdout || result.stderr);
                    }
